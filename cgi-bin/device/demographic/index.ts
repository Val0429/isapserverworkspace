        ...
        try {
            ...
            await Promise.all(
                _input.map(async (value, index, array) => {
                    try {
                        let device: IDB.Device = await Device.Create(Enum.EDeviceMode.demographic, value);
    ...
    async (data): Promise<OutputU> => {
        ...
        try {
            ...
            await Promise.all(
                _input.map(async (value, index, array) => {
                    try {
                        let device: IDB.Device = await Device.Update(Enum.EDeviceMode.demographic, value);
                    } catch (e) {
                        resMessages[index] = Parser.E2ResMessage(e, resMessages[index]);
