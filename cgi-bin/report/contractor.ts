...
action.post(async (data) => {
    ...
    for (let member of members) {
        ...
        for(let attendance of attendances){
            let monthDifferent =  moment.utc(moment(filter.End).diff(moment(attendance.attributes.attendanceStart.attributes.date_time_occurred))).month();
            if(member.lastDateOccured === attendance.attributes.dateOccurred)continue;
