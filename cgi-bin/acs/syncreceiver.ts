...
action.post<InputC, OutputC>({ inputType: "InputC" }, async (data) => {
    ...
    Log.Info(`info`, `postSyncReceiver ${message.receivers.map(x=>x.receivename).join(", ")}`, data.user, false);
...
action.put<InputU, OutputU>({ inputType: "InputU" }, async (data) => {
    ...
    let message = ParseObject.toOutputJSON(obj);
    Log.Info(`info`, `putSyncReceiver ${message.receivers.map(x=>x.receivename).join(", ")}`, data.user, false);
    /// 3) Output
    return ParseObject.toOutputJSON(obj);
});
...
action.delete<InputD, OutputD>({ inputType: "InputD" }, async (data) => {
    ...
    let message = ParseObject.toOutputJSON(obj);
    Log.Info(`info`, `deleteSyncReceiver ${message.receivers.map(x=>x.receivename).join(", ")}`, data.user, false);
    /// 3) Output
    return ParseObject.toOutputJSON(obj);
});
/// CRUD end ///////////////////////////////////
