...
let uniqueCount = 0;
export function filterFace(config: IFRSServiceConfig, compareCallback: (face: RecognizedUser | UnRecognizedUser) => void = null) {
    let { throttleKeepSameFaceSeconds, specialScoreForUnRecognizedFace } = config.frs;
    const defaultTargetScore = 0.6;
    const defaultTraceMilliSeconds = 15;
    const targetScore = specialScoreForUnRecognizedFace || defaultTargetScore;
    const traceMilliSeconds = (throttleKeepSameFaceSeconds || defaultTraceMilliSeconds) * 1000;
                ...
                switch (type) {
                    case UserType.UnRecognized:
                        ...
                        for (let i=indexTypeR.length-1; i>=0; --i) {
                            ...
                            if (prev.person_info.fullname === highestScore.fullname && highestScore.score >= targetScore) {
                    ...
                    case UserType.Recognized:
                        ...
                        for (let i=0; i<indexTypeU.length; ++i) {
                            ...
                            if (valrec.person_info.fullname === highestScore.fullname && highestScore.score >= targetScore) {
                                /// replace this face
                                valrec.timestamp = prev.timestamp;
                                valrec.valFaceId = prev.valFaceId;
                                tryCallback(valrec);
