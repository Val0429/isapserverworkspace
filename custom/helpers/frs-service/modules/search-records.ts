...
import { RecognizedUser, UnRecognizedUser, RequestLoginReason, UserType } from 'workspace/custom/helpers/frs-service/libs/core';
...
FRSService.prototype.searchRecords = function(starttime: Date, endtime: Date, pageSize: number = 20, times: number = 0): Subject<RecognizedUser | UnRecognizedUser> {
    ...
    let queueHandler = async () => {
        ...
        if (recog.finished === true && unrecog.finished === true) sj.complete();
    };
    ...
    (async () => {
        ...
        Observable.forkJoin(obRecog, obUnRecog).subscribe(() => {
            /// combine last
