...
class Service {
     ...
     *
     */
    public weatherSetting$: Rx.BehaviorSubject<IDB.ISettingWeather> = new Rx.BehaviorSubject(undefined);
    ...
    constructor() {
        ...
        this.weatherSetting$
            ...
            .subscribe({
                next: async (x) => {
                    await this.UpdateWeatherSetting();
                },
            });
    ...
    private async Search(): Promise<void> {
        try {
            ...
            tasks.push(this.SearchWeatherSetting());
    ...
    /**
     * Search Weather Setting
     */
    private async SearchWeatherSetting(): Promise<void> {
        try {
            let setting: IDB.SettingWeather = await new Parse.Query(IDB.SettingWeather).first().fail((e) => {
                throw e;
            });
            if (!setting) {
                this.weatherSetting$.next({
                    enable: false,
                    darksky: {
                        secretKey: '',
                    },
                ...
                this.weatherSetting$.next({
                    enable: setting.getValue('enable'),
                    darksky: setting.getValue('darksky'),
                });
            }
        } catch (e) {
            throw e;
     ...
     */
    private async UpdateWeatherSetting(): Promise<void> {
        try {
            let value = this.weatherSetting$.value;

            let setting: IDB.SettingWeather = await new Parse.Query(IDB.SettingWeather).first().fail((e) => {
                throw e;
            });
            if (!setting) {
                setting = new IDB.SettingWeather();
            }

            setting.setValue('enable', value.enable);
            setting.setValue('darksky', value.darksky);
