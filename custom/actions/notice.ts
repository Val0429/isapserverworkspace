...
class Action {
    private _notice$: Rx.Subject<Action.IMessageResident> = new Rx.Subject<Action.IMessageResident>();
    public get notice$(): Rx.Subject<Action.IMessageResident> {
        return this._notice$;
    }
    ...
    private ReplaceBody = (body: string, message: IDB.IMessageContent): string => {
        try {
            if (message.deadline) {
